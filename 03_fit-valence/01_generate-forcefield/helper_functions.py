"""
This module contains helper functions for manipulating parameters in force fields.
"""

import copy

def print_number_parameters(forcefield):
    bond_handler = forcefield.get_parameter_handler("Bonds")
    print(f"# Bonds: {len(bond_handler.parameters)}")
    angle_handler = forcefield.get_parameter_handler("Angles")
    print(f"# Angles: {len(angle_handler.parameters)}")
    torsion_handler = forcefield.get_parameter_handler("ProperTorsions")
    print(f"# Torsions: {len(torsion_handler.parameters)}")
    

def remove_constraints(force_field):
    """Remove the Constraints section from the OpenFF force field."""
...
def split_torsion_multiplicities(torsion_handler):
    """
    Split torsion multiplicities into separate parameters.
    """
...
def modify_cresset_torsions(torsion_handler):
    """
    Modify problematic torsions highlighted by Cresset.
    Add new biaryl torsion.
    """
    # cresset suggestion
